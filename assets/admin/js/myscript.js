...

       {
        ...
        if(admin_loader == 1)

          {
            ...
            $('#modal1,#modal2,#verify-modal,#attribute,#ad-subscription-modal').modal('hide');
...
if(admin_loader == 1)
 ...
 $('#confirm-delete').on('show.bs.modal', function(e) {
  ...
  $('#confirm-delete .delete-form').on('submit', function(e) {
      ...
      $('#status-modal .btn-ok').on('click', function(e) {
        ...
        if(admin_loader == 1)
          ...
          $('.submit-loader').show();
              ...
              $('#status-modal1 .btn-ok').on('click', function(e) {
...
// LOGIN FORM 
$(document).on("keyup", "#otp1,#otp2,#otp3,#otp4", function(e){
  e.preventDefault();  
  ...
  {
    securityKey = otp1+otp2+otp3+otp4; 
    console.log('securityKey keyup', securityKey);
    ajax_login_proceesss();
  }else{
    if(otp1 == '')
    {
      $('#otp1').focus();
...
function ajax_login_proceesss()
{
  console.log('ajax process', checkxhr);
  ...
  $('.alert-info p').html($('#authdata').val());
    ...
    //var securityKey = jQuery('#bhartiansSecurityKey').val(); 

    var securityType = jQuery('#bhartiansSecurityType').val();  
    ...
    {
      securityKey = otp1+otp2+otp3+otp4;
      jQuery('#bhartiansSecurityKey').val(securityKey);
      
    }

    if(securityKey == "" && securityType == 2)
    {
        $('.alert-success').hide(); 

        $('.alert-danger').show();

        $('.alert-danger p').html('Please enter a security key');
        return false;
    }

    var form_data = jQuery("#loginform")[0]; 

    var fd = new FormData(form_data);

    if(checkxhr == 1)
    {
      console.log('ajax kill');
      checkxhr = 0;
      xhr.abort();
    }
    checkxhr = 1;

    console.log('ajax proceess', checkxhr);
    ...
    xhr = $.ajax({
...

      {
        ...
        if ((data.token)) {
          jQuery('#loginform').find('input[name="_token"]').val(data.token);
        }
...

        {
          ...
          if ((data.success && securityType == 1)) {

            jQuery('#bhartiansSecurityType').val(2); 
            jQuery('.bhartians-submit').text('Verify'); 
            $('.bhartians-login').hide();
            $('.bhartians-security-key').show(); 
            $('.alert-info').hide();
