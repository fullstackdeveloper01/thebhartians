...
class CartController extends FrontBaseController
{
    ...
    public function addcart($id)
    {
        ...
        if(Auth::check()){
            ...
            $itemsArr= array(); 
            $itemsArr['items'] = json_encode($cart->items); 

            $is_cart = DB::table('cart_items')->where('user_id', $user_id)->count();

            if($is_cart > 0)
            { 
                DB::table('cart_items')->where('user_id', $user_id)->update($itemsArr); 
            }else{
                $itemsArr['user_id'] = $user_id;
                $itemsArr['ip_address'] =$this->getUserIpAddr();
                DB::table('cart_items')->insert($itemsArr);
            }
        }
    ...
    public function addbyone()
    {
        ...
        }else  if($cart->items[$itemid]['stock'] < 0)
        {
    ...
    public function reducebyone()
    {
        ...
        $oldCart = Session::has('cart') ? Session::get('cart') : null;
        $oldqty = $oldCart->items[$itemid]['qty'];
        if($oldqty <= 1)
        {
            return 0;
        } 
    ...
    public function removecart($id)
    {
        ...
        if (count($cart->items) > 0) {
            ...
            if(Auth::check()){
                $user_id =  Auth::user()->id;
                $itemsArr= array(); 
                $itemsArr['items'] = json_encode(array()); 
