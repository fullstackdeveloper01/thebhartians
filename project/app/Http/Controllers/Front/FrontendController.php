...
// CURRENCY SECTION ENDS
	...
	public function index(Request $request)
	{
        ...
        $active_slider = Category::select('id','name','slug','photo')->where('status', 1)->where('home_slider_active', 1)->get(); 
        
        $cat_products =array(); 
            
            if(!empty($active_slider))
            {
                foreach ($active_slider as $key => $cat_data) {
                    $dataArr = array();
                    $products = Product::where('category_id', $cat_data->id)->where('status', 1)->orderBy('id', 'desc')->skip(0)->take(10)->get();
                    $dataArr['id'] = $cat_data->id;
                    $dataArr['name'] = $cat_data->name;
                    $dataArr['photo'] = $cat_data->photo;
                    if(count($products)>0)
                    {
    ...
    public function extraIndex()
    {
        ...
        //Remove this commented code from add query
    ...
    public function autosearch($slug)
    {
        if(mb_strlen($slug,'UTF-8') > 1){
            $search = ' '.$slug;
            $prods = Product::where('status','=',1)->where('name', 'like', '%' . $search . '%')->orWhere('name', 'like', $slug . '%')->orderby('id','desc')->take(10)->get();
