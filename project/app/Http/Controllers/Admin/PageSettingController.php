...
class PageSettingController extends AdminBaseController
{
    ...
    public function homeupdate(Request $request)
    {
        ...
        if(!empty($categories))
        {
            foreach ($categories as $key => $cat) {
                ...
                if(in_array($cat->id, $main_categories))
                {
                    $home_slider_active = 1;
                }else{
                    $home_slider_active = 0;
                }

                $cat->update(array('home_slider_active' => $home_slider_active));
            }
        }
    ...
    {
        $data = Pagesetting::find(1); 
        $categories = Category::select('id','name','home_slider_active')->where('status', 1)->get(); 
        return view('admin.pagesetting.customize',compact('data', 'categories'));
    }
