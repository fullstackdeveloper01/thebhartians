...
class OrderController extends AdminBaseController
{
    ...
    public function orders(Request $request)
    {
        if($request->status == 'pending'){
            ...
            return view('admin.order.on_delivery');
    ...
    public function datatables($status)
    {
        ...
        elseif($status == 'on_delivery') {
            $datas = Order::where('status','=','on delivery')->latest('id')->get();
        }
    ...
    public function invoice2($id)
    {
        $order = Order::findOrFail($id);
        $cart = json_decode($order->cart, true);
        return view('admin.order.invoice2',compact('order','cart'));
    }
    ...
    public function printpage2($id)
    {
        $order = Order::findOrFail($id);
        $cart = json_decode($order->cart, true);
        return view('admin.order.print2',compact('order','cart'));
    }
