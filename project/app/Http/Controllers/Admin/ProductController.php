...
class ProductController extends AdminBaseController
{
    ...
    //*** POST Request
    public function uploadUpdateAjax(Request $request,$id)
    {

        //--- Validation Section
        $rules = [ 
          'image' => 'required|image|mimes:jpeg,png,jpg|max:4240',
        ];
        $validator = Validator::make($request->all(), $rules);
        if ($validator->fails()) {
          return response()->json(array('errors' => $validator->getMessageBag()->toArray()));
        ...
        $product_photo = $request->file('image'); 
        
        try {
            $image_name = time().\Str::random(8).'.'.str_replace(' ', '', $product_photo->getClientOriginalExtension());
            $product_photo->move('assets/images/products/',$image_name); 
    ...
    public function store(Request $request)
    {
        ...
        $rules = [
            //'photo'      => 'required',
            'product_photo' => 'required|image|mimes:jpeg,png,jpg|max:4240',
            'file'       => 'mimes:zip'
        ];
        ...
        if ($product_photo = $request->file('product_photo')) {
            ...
            if(empty($request->stock_check ))
            {
                ...
                $input['color'] = null;
                    ...
                    else
                    {
                        ...
                        if(!empty($request->size_qty) && is_array($request->size_qty))
                        {
        ...
        if(!empty($input['previous_price']))
        {
            $input['previous_price'] = ($input['previous_price'] / $sign->value);
            $input['price'] = ($input['previous_price'] / $sign->value);
            $input['base_price'] = $input['base_price'] / $sign->value; 

        }else{ 

              // Conert Price According to Currency
            $input['price'] = ($input['base_price'] / $sign->value);
            $input['previous_price'] = ($input['previous_price'] / $sign->value);
            $input['base_price'] = $input['base_price'] / $sign->value;
        }  
