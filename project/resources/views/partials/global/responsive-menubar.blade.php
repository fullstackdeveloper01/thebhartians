<div class="main-nav d-lg-block d-none py-3" style="background-color:#fff;">
                ...
                <nav class="navbar navbar-expand-lg nav-dark nav-primary-hover nav-line-active">
                    ...
                    <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        <ul class="navbar-nav ms-md-5">
                            ...
                            <li class="nav-item dropdown {{ request()->path() == '/products' ? 'active':''}}">
                                <a class="nav-link dropdown-toggle" href="{{ route('front.category') }}">{{ __('Products') }}</a>
                            </li>
                            <li class="nav-item dropdown mega-dropdown">
                                <a class="nav-link dropdown-toggle" href="{{ route('front.category') }}">{{ __('Collections') }}</a>
                ...
                <div class="margin-right-1 d-flex align-items-center justify-content-end h-100">
                    ...
                    <div class="sign-in my-account-dropdown position-relative">
                            ...
                            @if (Auth::check())
                                ...
                                $userShortName = substr($userFirstName, 0, 1);
                                $exdd_name = explode(' ', Auth::user()->name);

                                if(array_key_exists(1,$exdd_name))
                                {
                                    $my_secound_char = substr($exdd_name[1], 0, 1);
                                    $userFirstName = $exdd_name[0];
                                    $userShortName =  $userShortName.$my_secound_char;
                                 }else{
                                    $userShortName = substr(Auth::user()->name, 0, 2);
                                } 
                                $userShortName =  strtoupper($userShortName); 
                                
                                @endphp
                                <a href="{{ route('user-dashboard') }}" class="has-dropdown d-flex align-items-center w-100 text-whites text-decoration-none">
                                    <span class="initials mr-2">{{$userShortName}}</span>
                                <div class="user-menu-name">{{$userFirstName}}</div>
                                @else
                                <a href="{{ route('user-dashboard') }}" class="has-dropdown d-flex align-items-center text-dark text-decoration-none">
                                    <!-- <i class="flaticon-user-3 flat-mini mx-auto text-dark"></i> -->
                                    <div class="user-login-text">{{ ('My Account') }}</div>
                                @endif 
                            @else
                            <a href="{{ route('user.login') }}" class="has-dropdown d-flex align-items-center text-dark text-decoration-none">
                            <div class="user-login-text">{{ ('Login') }}</div>
                            @endif
                        </a>
                        <ul class="my-account-popup">
                            @if (Auth::check())

                            <li><a href="{{ route('user-dashboard') }}"><span class="menu-item-text">{{ ('My Account') }}</span></a></li>
                            ...
                            <li><a href="{{ route('user.register') }}"><span class="menu-item-text join" >{{ __('Registration') }}</span></a></li>
                            @endif
										...
										<div class="tab-pane fade show active woocommerce-Tabs-panel woocommerce-Tabs-panel--description" id="pills-push-menu" role="tabpanel" aria-labelledby="pills-push-menu-tab">
											<div class="push-navbar">
												<ul class="navbar-nav">
													...
													<li class="nav-item ">
														<a class="nav-link" href="{{ route('front.category') }}">{{ __('Collections') }}</a>
													</li>
													<!-- <li class="nav-item dropdown">
														<a class="nav-link dropdown-toggle" href="#">{{ __('Pages') }}</a>
														<ul class="dropdown-menu">
			...
			<div class="col-xxl-3 col-xl-4 col-lg-3 col-6 order-lg-3">
				<div class="d-flex align-items-center justify-content-end h-100 md-py-10">
					<div class="sign-in position-relative font-general my-account-dropdown">
							...
							@if (Auth::check())
                                @if(Auth::user()->name)
                                <a href="{{ route('user-dashboard') }}" class="has-dropdown d-flex w-100 align-items-center text-dark text-decoration-none" title="My Account">
                                    @if(!empty($userShortName))<span class="initials mr-2">{{$userShortName}}</span> @endif
							        <div class="user-menu-name">{{$userFirstName}}</div>
                                @else
                                ...
                                @endif 
                            @else
                            <a href="{{ route('user.login') }}" class="has-dropdown d-flex align-items-center text-dark text-decoration-none" title="My Account">
							<div class="user-login-text">{{ ('Login') }}</div>
							@endif
						</a>
						<ul class="my-account-popup">
							@if (Auth::check())
                                ...
                                <li><a href="{{ route('user.register') }}"><span class="menu-item-text sign-in joins" >{{ __('Registration') }}</span></a></li>
                                @endif
						</ul>
					</div>
					...
					<div class="refresh-view header-cart-1 ms-3">
